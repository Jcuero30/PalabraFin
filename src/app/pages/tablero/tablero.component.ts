  ...
  comprobarPalabra() {
    if (this.turno < this.palabraComponents.length) {
      ...
      if (currentPalabraComponent.allLettersFilled()) {
        if (currentPalabraComponent.VerificarPalabra()) {
          this.intentos++;
          const endTime = Date.now();
          const timeTaken = (endTime - this.startTime) / 1000;

          alert(`¡Palabra es correcta! Tiempo de juego: ${timeTaken} segundos. Intentos realizados: ${this.intentos}.`);
          ...
          this.palabraSer.GuardarJuego(datosJuego).subscribe(() => {
    ...
    if (this.turno >= this.palabraComponents.length) {
      ...
      alert(`Se han agotado todos los intentos. Tiempo de juego: ${timeTaken} segundos. Intentos realizados: ${this.intentos}. Volviendo al inicio.`);
